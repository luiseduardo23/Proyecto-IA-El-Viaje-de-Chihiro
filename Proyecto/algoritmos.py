...
def porCosto(matriz, xi, yi):
    ...
    while(NoMeta):
        if(matriz[target.elemento[0]][target.elemento[1]]!=3): #Verificamos si No es meta
            ...
            y=target.elemento[1]
            if((x<3) and not(estadoRepetido(x,y,target))):
            ...
            x = target.elemento[0]-1
            if((x>=0) and not(estadoRepetido(x,y,target))):        
                if(matriz[x][y]==0 or matriz[x][y]==3):
                    nn+=1
                    Arbol.agregarElemento(nn, [x,y], target.costoA+1,target, target.listaM.copy(), target.cantMA)
                    listaXYCostos.append([nn,target.costoA+1])
            ...
            if((y<len(matriz[0])) and not(estadoRepetido(x,y,target))):
                ...
                if(matriz[x][y]==5):
                    if (target.cantMA>=1):
                        ...
                        listaXYCostos.append([nn,target.costoA+2])
            y=target.elemento[1] -1              
            if ((y >= 0) and not(estadoRepetido(x,y,target))):
...
#Estado repetido?: Función booleana que es usada para conocer cuando es bueno devolverse, esto con el fin de repetir estados
#y crear nodos de manera innecesaria.

def estadoRepetido(x,y, nodoA): #nodoA = nodo Actual= target
    res = False
    if(nodoA.padre != None):
        if(nodoA.padre.elemento[0]==x and nodoA.padre.elemento[1]==y):
            if(nodoA.padre.cantMA==nodoA.cantMA):
               res= True
    return res
